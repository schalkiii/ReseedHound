...
| 对比维度       |  reseed-puppy   |    青蛙辅种     |       **SeedHound**       |
| -------------- | :-------------: | :-------------: | :-----------------------: |
| **运行模式**   | WebUI，串行执行 | WebUI，串行执行 |  纯 CLI，全链路异步并行   |
| **日志查看**   |   WebUI 内置    |   WebUI 内置    | 控制台 + 文件日志自动轮转 |
| **配置存储**   |     数据库      |     数据库      |      YAML 纯文本文件      |
| **配置方式**   |   WebUI 表单    |   WebUI 表单    |    文本编辑器直接编辑     |
| **批量处理**   |  逐个站点串行   |  逐个站点串行   |      多站点异步并行       |
| **大规模种子** | WebUI 渲染开销  | WebUI 渲染开销  |      无 UI 渲染开销       |
| **配置迁移**   |   数据库导出    |   数据库导出    |      文件复制即迁移       |
| **通知推送**   |     不支持      |      支持       |      飞书仪表盘卡片       |
