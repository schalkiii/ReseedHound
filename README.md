...
> 继承自 [reseed-puppy](https://github.com/yuusakurinosekai/reseed-puppy)，Hound（猎犬）象征更敏捷、更专业的辅种体验。Puppy 是小狗崽，Hound 是训练有素的猎犬——继承其基因，但嗅觉更敏锐、速度更快、自动化程度更高。

最简高性能 Python 辅种引擎 —— 跨 PT 站点间的高效种子调度工具，猎犬般自动嗅探并匹配可辅种任务。

## 为什么选择 SeedHound

| 痛点               |       reseed-puppy       |            青蛙辅种            |             **SeedHound**             |
| ------------------ | :----------------------: | :----------------------------: | :-----------------------------------: |
| **运行模式**       |   WebUI 必须，串行执行   |      WebUI 必须，串行执行      |        纯 CLI，全链路异步并行         |
| **日志查看**       | WebUI 内置，种子多时卡死 |      WebUI 内置，不便排查      |   控制台实时输出 + 文件日志自动轮转   |
| **配置存储**       |  数据库，不易迁移/备份   |    数据库，遇过多次配置丢失    |      YAML 纯文本文件，复制即备份      |
| **配置方式**       |      WebUI 表单填写      |         WebUI 表单填写         |    文本编辑器直接编辑，所见即所得     |
| **批量处理**       |     逐个站点串行查询     |        逐个站点串行查询        |  94 条查询链路并行，耗时 = 最慢站点   |
| **种子多时稳定性** |   大量种子导致页面卡死   |        同样存在性能瓶颈        |   万级种子秒级扫描，无 UI 渲染开销    |
| **配置迁移**       |      导出数据库困难      | 曾多次出现无法保存配置只能重装 | `sites.yaml` + `config.yaml` 直接复制 |
| **通知方式**       |            无            |               无               | 飞书仪表盘卡片推送，关键指标一目了然  |
