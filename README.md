...
SeedHound 在辅种流水线的不同阶段部署了五层过滤，逐层拦截已知冗余，最小化站点 API 压力和网络下载开销：
...
│  pieces_hash 已成功辅种到该站 → 不发送 API 查询               │
│                                                               │
...
┌─ 阶段3: 下载前 ───────────────────────────────────────────────┐
│                                                               │
│  第三层 · valid_reseeded 检查                                  │
│  (pieces_hash, 站点, torrent_id) 在缓存且种子仍在 qB → 跳过    │
│                                                               │
│  第四层 · tracker 同站检查（兜底）                              │
│  pieces_hash 在 qB 中且 tracker 域名匹配 → 跳过下载            │
│                                                               │
│  效果: 避免下载已在 qB 中存在的种子                             │
└───────────────────────────────────────────────────────────────┘
                              ↓ 未命中
...
> **关键设计**：前三层过滤在下载前完成，无需消耗站点下载带宽。第一、二层在 API 查询阶段就将已知种子排除，从源头减少 API 请求量。第三、四层在下载前再次校验，防止不同 torrent_id 的同内容种子被重复下载。第五层为兜底，确保任何漏网之鱼只被下载一次。
